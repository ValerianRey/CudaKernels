...
TEST(GemmCutlass, SingleElement)
{
	...
	gemmCutlassGPU(1, 1, 1, 2.f, A.get(), B.get(), 0.5f, C.get());

	// alpha * A@B + beta * C = 2 * 15 + 0.5 * 100 = 80
	EXPECT_FLOAT_EQ(C[0], 80.f);
}

TEST(GemmCutlass, BetaZeroIgnoresNanInC)
{
	int m = 5, n = 4, p = 3;
	...
	gemmCutlassGPU(m, n, p, 1.f, A.get(), B.get(), 0.f, C.get());
...
TEST(GemmCutlass, ZeroReductionDimOnlyScalesC)
{
	int m = 4, p = 3;
	...
	gemmCutlassGPU(m, 0, p, 5.f, A.get(), B.get(), 2.f, C.get());

	for (int i = 0; i < m * p; i++)
		EXPECT_FLOAT_EQ(C[i], 2.f * static_cast<float>(i + 1));
}

TEST(GemmCutlass, DimensionsNotMultipleOfTileSize)
{
	// Deliberately not multiples of CUTLASS's default 128x128x8 threadblock
	// tile, to exercise its own boundary handling.
	int m = 70, n = 50, p = 90;
	CudaManagedArray<float> A(m * n);
	CudaManagedArray<float> B(n * p);
	CudaManagedArray<float> C(m * p);
...
TEST(GemmCutlass, MatchesHandWrittenGemm)
{
	int m = 137, n = 91, p = 113;
	CudaManagedArray<float> A(m * n);
	CudaManagedArray<float> B(n * p);
	CudaManagedArray<float> cutlassOut(m * p);
	CudaManagedArray<float> handWrittenOut(m * p);

	fillRandom(m * n, A.get(), 7);
	fillRandom(n * p, B.get(), 8);

	float alpha = 0.9f, beta = 0.f;

	gemmCutlassGPU(m, n, p, alpha, A.get(), B.get(), beta, cutlassOut.get());
	gemmGPU(m, n, p, alpha, A.get(), B.get(), beta, handWrittenOut.get());

	expectAllClose(m * p, handWrittenOut.get(), cutlassOut.get());
}

TEST(GemmCutlass, LargeRandomInput)
{
